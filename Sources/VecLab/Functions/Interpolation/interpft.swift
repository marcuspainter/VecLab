...
public func interpft(_ x: RealArray, _ n: Int) -> RealArray {
    ...
    if mod(length(x), 2) != 0 {
        ...
        let z = zeros(N * Int(U) - N)
        let t1 = X[0..<(N / 2)]
        let t2 = SplitComplexArray([X.real[N / 2 + 1]], [X.imag[N / 2 + 1]]) * 0.5
        let t3 = SplitComplexArray(z, z)
        let t4 = SplitComplexArray([X.real[N / 2 + 1]], [X.imag[N / 2 + 1]]) * 0.5
        let t5 = X[N / 2 + 1..<N]
