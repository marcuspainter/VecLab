...
public func circshift(_ x: RealDoubleArray, _ k: Int) -> RealDoubleArray {
    let n = x.count
...
///   - k: Shift amount.
/// - Returns: Circularly shifted array
public func circshift(_ x: ComplexDoubleArray, _ k: Int) -> ComplexDoubleArray {
...
///   - k: Shift amount.
/// - Returns: Circularly shifted array
public func circshift(_ x: RealFloatArray, _ k: Int) -> RealFloatArray {
    let n = x.count
    if n == 0 { return x } // Edge case: empty array
    let shift = ((k % n) + n) % n
    if shift == 0 { return x } // Edge case: no effective shift
...
///   - k: Shift amount.
/// - Returns: Circularly shifted array
public func circshift(_ x: ComplexFloatArray, _ k: Int) -> ComplexFloatArray {
    let r = circshift(x.0, k)
    let i = circshift(x.1, k)
    return (r, i)
}
