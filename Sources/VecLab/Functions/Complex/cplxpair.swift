...
public func cplxpair(_ A: ComplexDoubleArray, tol: RealDouble = 1e-6) -> ComplexDoubleArray {
    ...
    var realPaired = RealDoubleArray(repeating: 0, count: A.0.count)
    var imagPaired = RealDoubleArray(repeating: 0, count: A.1.count)
...
func isApproximatelyEqualTo(_ complex: ComplexDoubleArray, _ other: ComplexDouble, tol: RealDouble, at index: Int) -> Bool {
...
///   - tol: tolerance.
/// - Returns:An array of sorted pairs
public func cplxpair(_ A: ComplexFloatArray, tol: RealFloat = 1e-6) -> ComplexFloatArray {
    // Ensure that a one-element array returns itself.
    guard A.0.count > 1 else { return ([A.0.first!], [A.1.first!]) }

    // Allocate memory for the output arrays.
    var realPaired = RealFloatArray(repeating: 0, count: A.0.count)
    var imagPaired = RealFloatArray(repeating: 0, count: A.1.count)
...
func isApproximatelyEqualTo(_ complex: ComplexFloatArray, _ other: ComplexFloat, tol: RealFloat, at index: Int) -> Bool {
    return abs(complex.0[index] - other.0) <= tol && abs(complex.1[index] + other.1) <= tol
}
