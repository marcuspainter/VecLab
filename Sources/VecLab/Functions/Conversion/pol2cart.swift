...
public func pol2cart(_ theta: RealArray, _ rho: RealArray, _ z: RealArray) -> (x: RealArray, y: RealArray, z: RealArray) {
    let n = theta.count
    assert(rho.count == n && z.count == n, "Arrays must be the same size, theta: \(theta.count), rho: \(rho.count), z: \(z.count) ")
    var x = [Real](repeating: Real(0), count: n)
    var y = x   // Zeros
    var zz = x  // Zeros
