...
func XCTAssertEqual(
    _ expression1: ([Double], [Double]),
    _ expression2: ([Double], [Double]),
    ...
    XCTAssertEqual(
        ...
        expression2.1,
        accuracy: accuracy,
        "Imaginary part mismatch: \(message())",
        file: file,
        line: line
    )
}
...
func XCTAssertEqual(
    ...
    if expression1.count != expression2.count {
        XCTFail(
            "Size mismatch: \(expression1.count) is not equal to \(expression2.count). \(message())",
            file: file,
            line: line
        )
    }
    ...
    for (index, (value1, value2)) in zip(expression1, expression2).enumerated() {
        if abs(value1 - value2) > accuracy {
            XCTFail(
                "Element \(index) mismatch: \(value1) is not equal to \(value2). \(message())",
                file: file,
                line: line
            )
        }
...
func XCTAssertEqual(
    _ expression1: ([Float], [Float]),
    _ expression2: ([Float], [Float]),
    accuracy: Float = Float(epsilon),
    _ message: @autoclosure () -> String = "",
    file: StaticString = #filePath,
    line: UInt = #line
) {
...
func XCTAssertEqual(
    ...
    XCTAssert(
        expression1.real.count == expression2.real.count && expression1.count == expression2.count,
        "Not same size"
    )
    XCTAssertEqual(
        expression1.real,
